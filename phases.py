...
def read_from(file):
    ...
    read_input_text = input_data.read()
...
def get_dcf_commands(text_file):
    ...
    mo_all_commands_text = rx_all_commands.search(
        input_text
    )  # NOTE: mo stands for matched object
    try:
...
def text_single_phase(input_file, phase):
    ...
    rx_phase = r"(PHASE {0} INITIALIZED.*?) PHASE {1} INITIALIZED".format(
        phase, phase + 1
    )
