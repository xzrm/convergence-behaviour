...
rx_dict = {
    ...
    "load_case": re.compile(
        r"\s*LOAD INCREMENT:  LOADING\(\s*(?P<load_case>\d*)\) \*   \d*\.\d*E[-|+]\d*"
    ),
    "norm": re.compile(
        r"\s*STEP\s*\d*\s*:\s*(?P<norm>[A-Z]* NORM)\s*=  \d*\.\d*E[-|+]\d*\s*TOLERANCE =  (?P<norm_tolerance>\d*\.\d*E[-|+]\d*)"
    ),
    "rel_displ_var": re.compile(
        r"\s*RELATIVE DISPLACEMENT VARIATION =  (?P<rdv>\d*\.\d*E[-|+]\d*)         CHECK = (?P<rdv_check>\w*)"
    ),
    "rel_energy_var": re.compile(
        r"\s*RELATIVE ENERGY VARIATION       =  (?P<rev>\d*\.\d*E[-|+]\d*)         CHECK = (?P<rev_check>\w*)"
    ),
    "rel_force_var": re.compile(
        r"\s*RELATIVE OUT OF BALANCE FORCE   =  (?P<rfv>\d*\.\d*E[-|+]\d*)         CHECK = (?P<rfv_check>\w*)"
    ),
    "rel_residu_var": re.compile(
        r"\s*RELATIVE RESIDU VARIATION       =  (?P<rrv>\d*\.\d*E[-|+]\d*)         CHECK = (?P<rrv_check>\w*)"
    ),
    # 'no_conv': re.compile(r'\s*STEP\s*(?P<step_n>\d*)\s*TERMINATED,\s*NO\s*CONVERGENCE\s*AFTER\s*\d*\s*ITERATION(S)?'),
    "conv": re.compile(
        r"\s*STEP\s*(?P<step_n>\d*)\s*TERMINATED,\s*CONVERGENCE\s*AFTER\s*(?P<itera>\d*)\s*ITERATION(S)?"
    ),
    "itera": re.compile(
        r"\s*STEP\s*(?P<step_n>\d*)\s*TERMINATED,\s*(NO\s*)?CONVERGENCE\s*AFTER\s*\d*\s*ITERATION(S)?"
    ),
    "cumulative_react": re.compile(
        r"     CUMULATIVE REACTION:         FORCE X        FORCE Y          FORCE Z"
    ),
    "cumulative_react_values": re.compile(
        r"\s*(?P<force_x>(-)?\d*\.\d*D[-|+]\d*)\s*(?P<force_y>(-)?\d*\.\d*D[-|+]\d*)\s*(?P<force_z>(-)?\d*\.\d*D[-|+]\d*)"
    ),
}
...
class SingleStepData:
    ...
    def __str__(self):
        s = "STEP NO. {}\nITERATIONS {}\nCONVERGED STEP     {}\nDISPLACEMENT NORM: {}\nFORCE NORM: {}\nENERGY NORM: {}\nLOAD CASE: {}\nGOVERNING NORM: {}".format(
            ...
            str(self.converged),
            str(self.displacement_norm),
            str(self.force_norm),
            str(self.energy_norm),
            self.load_case,
            self.governing_norm,
        )
...
class Convergence:
    ...
    def convergence_check(self, step_obj):
        ...
        if step_obj.converged:
            try:
                ...
                self.converged_norm_this.append(
                    (step_obj.total_iterations, last_variation)
                )
            except KeyError:
                self.converged_norm_other.append(
                    (step_obj.total_iterations, last_variation)
                )
        else:
    ...
    def variations(self):
        self.iterations_steps = [
            getattr(step_obj, self.norm) for step_obj in self.step_objects
        ]
